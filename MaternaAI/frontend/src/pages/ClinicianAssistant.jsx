...
const modes = {
  triage: {
    id: 'triage',
    name: 'Rapid Triage',
    desc: 'Immediate danger signs and escalation guidance',
    colorClass: 'danger',
  ...
  vitals: {
    id: 'vitals',
    name: 'Vitals Watch',
    desc: 'BP, glucose, fetal movement summary checks',
  ...
  followup: {
    ...
    desc: 'Outreach scripts and care continuity planning',
    colorClass: 'success',
  ...
  community: {
    ...
    desc: 'Field updates, transport, and referral routing',
    colorClass: 'purple',
...
const ClinicianAssistant = () => {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        ...
        setErrorMessage('Failed to restore clinician logs. Using offline mode.');
      } finally {
  ...
  const getClinicianProfile = () => {
    return {
      name: user?.name || 'Clinician',
      role: user?.role || 'clinician',
      location: user?.location || 'Facility not specified',
      department: user?.department || 'Maternal Health',
      panel_size: user?.panel_size || 0
    };
  };
    ...
    if (SpeechRecognition) {
      try {
        ...
        rec.onresult = (event) => {
          ...
          if (text) {
            console.log("Clinician client-side captured transcript:", text);
            clientTranscriptionRef.current = text;
          }
        };
        rec.onerror = (e) => {
          console.warn("Clinician client-side SpeechRecognition error:", e.error);
        };
        rec.start();
      ...
      <div className="bg-white rounded-2xl p-5 border border-primary-mauve/10 shadow-premium flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-text-dark font-sans flex items-center gap-2">
            <Stethoscope className="w-6 h-6 text-primary-mauve" />
            Clinician AI Assistant
          </h1>
          <p className="text-xs font-semibold text-text-muted mt-1">
            Voice + chat guidance for triage, follow-up scripts, and operational decision support.
          </p>
        </div>
          ...
          <div className="bg-white border border-primary-mauve/10 rounded-2xl p-6 shadow-premium flex flex-col items-center justify-center text-center flex-1 relative overflow-hidden min-h-[300px]">
            ...
            <div className="space-y-1.5 relative z-10">
              ...
              <p className="text-[11px] font-medium text-text-muted max-w-xs mx-auto leading-relaxed">
                {isRecording
                  ? 'Speak in Bengali or English. Tap again when you finish dictation.'
                  : 'Secure text-to-speech generates on-demand clinical summaries and scripted outreach replies.'}
              </p>
            </div>
          ...
          <div className="bg-white border border-primary-mauve/10 rounded-2xl p-5 shadow-premium space-y-3.5">
            <div className="flex items-center justify-between border-b border-primary-mauve/5 pb-2.5">
              <div className="flex items-center gap-2 text-text-dark">
                ...
                <h4 className="text-xs font-black uppercase tracking-wider">Clinical Context Envelope</h4>
              </div>
            ...
            <p className="text-[10px] font-medium text-text-muted leading-relaxed">
              The assistant uses your facility and role metadata to prioritize triage scripts and care pathways.
            </p>
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] font-bold text-text-dark">
              <div className="flex items-center gap-1.5 p-2 rounded-lg bg-bg-rose-white border border-primary-mauve/5">
                <Building2 className="w-3.5 h-3.5 text-primary-mauve" />
                <span>{user?.department || 'Maternal Health'}</span>
              </div>
              <div className="flex items-center gap-1.5 p-2 rounded-lg bg-bg-rose-white border border-primary-mauve/5">
                <MapPin className="w-3.5 h-3.5 text-info" />
                <span>{user?.location || 'Facility not specified'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-white border border-primary-mauve/10 rounded-2xl shadow-premium flex flex-col h-[650px] overflow-hidden">
          ...
          >
            <input
              ...
              placeholder="Ask for a triage summary or follow-up script..."
              disabled={isProcessing}
